...
def test_parse_active_only(mem_dir):
    _write(
        ...
        [{"cycle": 1, "goal": "g1"}, {"cycle": 2, "goal": "g2"}],
    )
    out = journal._parse_journal_entries()
    assert [e["cycle"] for e in out] == [2, 1]


def test_parse_merges_archive(mem_dir):
    _write(mem_dir / "journal.json", [{"cycle": 5, "goal": "new"}])
    _write(
        mem_dir / "journal-archive.json",
        [{"cycle": 1, "goal": "old"}, {"cycle": 2, "goal": "older"}],
    )
    out = journal._parse_journal_entries()
    assert [e["cycle"] for e in out] == [5, 2, 1]


def test_parse_active_takes_precedence(mem_dir):
    _write(mem_dir / "journal.json", [{"cycle": 1, "goal": "ACTIVE"}])
    _write(mem_dir / "journal-archive.json", [{"cycle": 1, "goal": "ARCHIVED"}])
    out = journal._parse_journal_entries()
    assert len(out) == 1
    assert out[0]["goal"] == "ACTIVE"


def test_parse_handles_non_list_active(mem_dir):
    _write(mem_dir / "journal.json", {"oops": "not a list"})
    _write(mem_dir / "journal-archive.json", [{"cycle": 1, "goal": "g"}])
    out = journal._parse_journal_entries()
    assert [e["cycle"] for e in out] == [1]


def test_parse_handles_non_list_archive(mem_dir):
    _write(mem_dir / "journal.json", [{"cycle": 1, "goal": "g"}])
    _write(mem_dir / "journal-archive.json", {"oops": True})
    out = journal._parse_journal_entries()
    assert [e["cycle"] for e in out] == [1]
