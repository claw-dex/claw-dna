...
def _build_items(goals, inbox, outbox, reminders, filter_cat):
    """Build a unified list of compact display items, newest-first."""
    items = []
    reminder_task_ids = {r.get("task_id") for r in reminders if r.get("task_id")}
    ...
    if filter_cat in ("All", "Inbox"):
        for item in inbox:
            # Skip inbox entries generated by a reminder fire — those are
            # already represented by the reminder row to avoid double-listing.
            if item.get("task_id") in reminder_task_ids:
                continue
            cmd_t = item.get("type", "?")
            items.append(
                {
            ...
            items.append(
                {
                    "cat": "reminder",
                    "icon": "🔔",
                    "badge_text": r.get("schedule_type") or "reminder",
                    "badge_color": "#FF9800",
...
def _render_detail(item):
    ...
    if cat == "goal":
        ...
        content = raw.get("content") or ""
        st.markdown(content)
        ts = str(raw.get("ts", ""))[:19].replace("T", " ")
        if ts:
            st.caption(f"Fired: {ts}")
...
def render():
    ...
    items = _build_items(goals, inbox, outbox, reminder_rows, filter_cat)
        ...
        for item in display_items:
            ...
            with col_exp:
                with st.expander(label, expanded=item["pinned"]):
                    _render_detail(item)
            with col_btn:
                if item["cat"] == "reminder":
                    task_id = item["raw"].get("task_id", "")
                    if st.button("Done", key=f"rem_done_{task_id}"):
                        result = delete_scheduled_task(task_id) or {}
                        if result.get("ok"):
                            st.toast(f"Reminder cleared: {task_id}")
                        else:
                            st.toast(
