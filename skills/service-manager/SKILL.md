...
description: Manage agent background processes. Use to register a new background service (start), stop one (stop), check its status (status), health-check all registered services (health), list all entries (list), or remove dead/stale entries (cleanup). Example: starting a webhook listener or a custom API service.
---
