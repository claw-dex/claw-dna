...
uv run python scripts/memory_repair.py

# Diagnose without touching files
uv run python scripts/memory_repair.py --dry-run

# Create fresh .backup copies of all healthy files
uv run python scripts/memory_repair.py --backup

# Silent repair for scripting
uv run python scripts/memory_repair.py --quiet

# JSON output for automation
uv run python scripts/memory_repair.py --json
```
