...
uv run python scripts/memory-repair.py

# Diagnose without touching files
uv run python scripts/memory-repair.py --dry-run

# Create fresh .backup copies of all healthy files
uv run python scripts/memory-repair.py --backup

# Silent repair for scripting
uv run python scripts/memory-repair.py --quiet

# JSON output for automation
uv run python scripts/memory-repair.py --json
```
